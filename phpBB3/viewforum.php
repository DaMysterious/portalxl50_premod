...
//		$s_type_switch_test = ($row['topic_type'] == POST_ANNOUNCE || $row['topic_type'] == POST_GLOBAL) ? 1 : 0;
		...
		$sql = 'SELECT t.*, u.*
			...
			WHERE t.topic_last_poster_id = u.user_id
				AND ' . $db->sql_in_set('u.user_id', $row['topic_last_poster_id']);
		$result = $db->sql_query($sql);
		$flag_last = $db->sql_fetchrow($result);
		
		if ($user->data['user_id'] != ANONYMOUS)
		{
			$flag_title = $flag_last_img = $flag_img_src = '';
			get_user_flag($flag_last['user_country_flag'], $flag_last['user_country'], $flag_title, $flag_last_img, $flag_img_src);
		}
		$db->sql_freeresult($result);
		...
		$sql = 'SELECT t.*, u.*
			...
			WHERE t.topic_poster = u.user_id
				AND ' . $db->sql_in_set('u.user_id', $row['topic_poster']);
		$result = $db->sql_query($sql);
		$flag_first = $db->sql_fetchrow($result);
		
		if ($user->data['user_id'] != ANONYMOUS)
		{
			$flag_title = $flag_first_img = $flag_img_src = '';
			get_user_flag($flag_first['user_country_flag'], $flag_first['user_country'], $flag_title, $flag_first_img, $flag_img_src);
		}
		$db->sql_freeresult($result);
		// Country Flags Version 3.0.6
		...
		$template->assign_block_vars('topicrow', array(
			...
			'TOPIC_AUTHOR_FULL'			=> get_username_string('full', $row['topic_poster'], $row['topic_first_poster_name'], $row['topic_first_poster_colour']) . ' ' . $flag_first_img,
...
//			'LAST_POST_AUTHOR_FULL'		=> get_username_string('full', $row['topic_last_poster_id'], $row['topic_last_poster_name'], $row['topic_last_poster_colour']),
			...
			// Ajax Userinfo MOD start 
			'LAST_POST_AUTHOR_FULL'		=> $last_post_author . ' ' . $flag_last_img,
			// Ajax Userinfo MOD end 
...
//		$s_type_switch = ($row['topic_type'] == POST_ANNOUNCE || $row['topic_type'] == POST_GLOBAL) ? 1 : 0;
		...
		if ((!isset($topic_att)) || (!isset($topic_img)))	
		{
		   $topic_att = array();
		   $topic_img = array();
		}
	...
	// Topic short description and READ MORE / 4seven / 2010	
	include($phpbb_root_path . 'includes/topic_short_description.' . $phpEx);
	// Topic short description and READ MORE / 4seven / 2010		
}
...
include($phpbb_root_path . 'includes/first_x_in_forum_index.' . $phpEx);
// 4seven / First Topic [pics] on Forum Index  / 2010

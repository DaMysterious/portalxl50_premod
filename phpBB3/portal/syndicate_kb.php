...
* @version $Id: syndicate_kb.php,v 1.3 2010/10/16 portalxl group Exp $
*
...
$kb_topic = ($script_name != '') ? $script_name . '/kb.' . $phpEx : '/kb.'. $phpEx;
...
//      {
		$rdf .= "<item>";
		$rdf .= "<title>" . $row['article_title'] . "</title>";
		$rdf .= "<link>" . $kb_topic_url . "?a=" . $row["article_id"] . "</link>";
        $rdf .= "<pubDate>" . date("D, d M Y H:G:s O", $row['article_time']) . "</pubDate>";
		$rdf .= "<description>" . generate_text_for_display($row['article_desc'], ENT_QUOTES, 'utf-8', '') . "</description>";
		$rdf .= "</item>";
      }
//	}

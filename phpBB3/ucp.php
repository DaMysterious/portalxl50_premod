...
switch ($mode)
{
	...
	case 'switch_perm':
		...
		// phpBB Arcade Start
		$message = sprintf($user->lang['PERMISSIONS_TRANSFERRED'], $user_row['username']) . $arcade_message . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$phpbb_root_path}index.$phpEx") . '">', '</a>');
		// phpBB Arcade End
		trigger_error($message);

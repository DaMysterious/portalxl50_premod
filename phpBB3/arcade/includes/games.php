...
*
* @package arcade
* @version $Id: games.php 1663 2011-09-22 12:09:30Z killbill $
...
make_arcade_jumpbox($arcade->url(), $cat_id);

$sort_key	= request_var('sk', (!empty($user->data['games_sort_order']) && !$arcade_config['override_user_sort']) ? $user->data['games_sort_order'] : $arcade_config['games_sort_order']);
$sort_dir	= request_var('sd', (!empty($user->data['games_sort_dir']) && !$arcade_config['override_user_sort']) ? $user->data['games_sort_dir'] : $arcade_config['games_sort_dir']);
...
$sql_array = array(
	'SELECT'	=> 'g.*, u.username, u.user_colour, u.user_id, s.score, s.score_date, r.game_rating, c.cat_name, c.cat_download, c.cat_cost, c.cat_reward, c.cat_use_jackpot',
	...
	'LEFT_JOIN'	=> array(
		...
		array(
			...
			'ON'	=> 's.game_id = g.game_id AND s.user_id = ' . (int) $user->data['user_id']
		),
		array(
...
$games_per_page = (!empty($user->data['games_per_page'])) ? $user->data['games_per_page'] : $arcade_config['games_per_page'];
...
switch($mode)
{
	...
	case 'fav':
		...
		$template->assign_block_vars('navlinks', array(
			...
			'U_VIEW_FORUM'		=> $arcade->url("mode=fav")));

		$total_games = $arcade->get_total('fav', $user->data['user_id']);
		if (!$total_games)
		{
			trigger_error($user->lang['ARCADE_NO_GAME_FAVS'] . $arcade->back_link());
		}
	...
	case 'search':
		...
		if ($search_id == 'newgames')
		{
			...
			$new_games_delay = (time() - ($arcade_config['new_games_delay'] * 86400));
			$total_games = $arcade->get_total('search_newgames', $new_games_delay);
			if (!$total_games)
			{
				trigger_error($user->lang['ARCADE_SEARCH_NO_MATCHES'] . $arcade->back_link());
			}

			$user->add_lang('search');

			$sql_array['WHERE'] = 'g.game_installdate >= ' . (int) $new_games_delay . '
									AND ' . $db->sql_in_set('g.cat_id', $arcade->games_search_protection(), true, true) . '
									AND ' . $db->sql_in_set('g.cat_id', $arcade->get_permissions('c_view'), false, true);
		...
		else
		{
			...
			$interval = ($user->data['user_id'] == ANONYMOUS) ? $arcade_config['search_anonymous_interval'] : $arcade_config['search_interval'];
			if ($interval && !$auth_arcade->acl_get('u_ignoreflood_search') && $user->data['user_arcade_last_search_term'] != $term)
			{
				$time = time() - $interval;
				if ($user->data['user_arcade_last_search'] > time() - $interval)
				{
					trigger_error(sprintf($user->lang['ARCADE_NO_SEARCH_TIME'], $arcade->time_format($interval, true), $arcade->time_format($user->data['user_arcade_last_search'] - $time, true)) . $arcade->back_link());
				}

				$sql_ary = array(
					'user_arcade_last_search'			=> time(),
					'user_arcade_last_search_term'		=> $term,
				);
	...
	case 'cat':
		...
		if (!$cat_data)
		{
			trigger_error($user->lang['NO_CAT_ID'] . $arcade->back_link());
		}
		...
		if (!$auth_arcade->acl_gets('c_list', 'c_view', $cat_id) || ($cat_data['cat_type'] == ARCADE_LINK && $cat_data['cat_link'] && !$auth_arcade->acl_get('c_view', $cat_id)))
		{
			...
			{
				trigger_error($user->lang['NO_PERMISSION_ARCADE_VIEW'] . $arcade->back_link());
			}

			login_box('', $user->lang['LOGIN_VIEWARCADE']);
		}

		if (!empty($cat_data['cat_games_per_page']) && empty($user->data['games_per_page']))
		{
			$games_per_page = $cat_data['cat_games_per_page'];
		}

		// Change category style if needed
		if ($cat_data['cat_style'])
		{
			$user_style = (!$config['override_user_style']) ? $user->data['user_style'] : $config['default_style'];
			if ($user_style != $cat_data['cat_style'])
			{
		...
		if ($cat_data['cat_age'])
		{
			$arcade->verify_age('cat', $cat_data['cat_age']);
		}
...
display_arcade_header($arcade_config['welcome_cats'], $arcade_config['search_cats'], $arcade_config['links_cats']);
...
$template->assign_vars(array(
	...
	'S_ARCADE_ACTION'		=> $arcade->url("$pagination_mode&amp;start=$start"),
));
...
if ($total_games)
{
	...
	if ($row = $db->sql_fetchrow($result))
	{
		do
		{
			...
			$gamesrow = array(
				...
				'S_GAME_PLAY_POPUP' 	=> ($auth_arcade->acl_get('c_popup', $row['cat_id'])) ? true : false,
				...
				'U_CAT'					=> $arcade->url("mode=cat&amp;c={$row['cat_id']}"),
				...
				'U_GAME_PLAY_POPUP'		=> $arcade->url("mode=popup&amp;g={$row['game_id']}"),
				'U_GAME_DOWNLOAD'		=> $arcade->url("mode=download&amp;g={$row['game_id']}"),
				'U_GAME_HIGHSCORES'		=> $arcade->url("mode=stats&amp;g={$row['game_id']}"),
				...
				'GAME_RATING_IMG'		=> $arcade->set_rating_image($row, $mode),
				'GAME_FAV_IMG'			=> $arcade->set_fav_image($game_fav_data, $row['game_id'], $mode),
	...
	$template->assign_vars(array(
		...
		'PAGINATION'	=> ($games_per_page > 0) ? generate_pagination($arcade->url($pagination_mode), $total_games, $games_per_page, $start) : false,
		)
	);
}

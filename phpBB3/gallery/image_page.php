...
if (!empty($phpbb_seo->seo_opt['zero_dupe']['on'])) {
	...
	$sort_order = request_var('sort_order', phpbb_gallery_config::get('default_sort_dir'));
	...
	$phpbb_seo->seo_chk_dupe('', '', phpbb_gallery_url::path('gallery'));
}
...
$phpbb_seo->prepare_url('pic', $image_data['image_name'], $image_id, $phpbb_seo->seo_url['album'][$album_id]);
$image_flat_url = phpbb_gallery_url::append_sid('image_page', "album_id=$album_id&amp;image_id=$image_id", true, '');
$image_flat_src = phpbb_gallery_url::append_sid('image', "album_id=$album_id&amp;image_id=$image_id", true, '');
$image_flat_src_thumb = phpbb_gallery_url::append_sid('image', "album_id=$album_id&amp;image_id=$image_id&amp;mode=thumbnail", true, '');
$image_bbcode_src = "[url=$image_flat_url][img]{$image_flat_src_thumb}[/img][/url]";
...
$template->assign_vars(array(
	...
	'IMAGE_BBCODE'		=> ($config['allow_bbcode']) ? "[img]{$image_flat_src}[/img]" : '',
	...
	'IMAGE_IMGURL_BBCODE'	=> (phpbb_gallery_config::get('disp_image_url')) ? $image_bbcode_src : '',
	...
	'IMAGE_URL'			=> (phpbb_gallery_config::get('disp_image_url')) ? $image_flat_url : '',
	// www.phpBB-SEO.com SEO TOOLKIT END
...
if (!empty($seo_meta)) {
	$seo_meta->collect('description', $image_data['image_name'] . ' : ' . (!empty($image_data['image_desc']) ? $image_data['image_desc'] : (!empty($album_data['album_desc']) ? $album_data['album_desc'] : $config['site_desc'])) . ' - ' . $album_data['album_name']);

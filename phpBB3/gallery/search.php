...
if ($keywords || $username || $user_id || $search_id || $submit)
{
	...
	// www.phpBB-SEO.com SEO TOOLKIT BEGIN
	$u_search = phpbb_gallery_url::append_sid('search', $u_search);
	// www.phpBB-SEO.com SEO TOOLKIT END
	...
	if ($sql_where)
	{
		...
		else
		{
			...
			while ($commentrow = $db->sql_fetchrow($result))
			{
				...
				// www.phpBB-SEO.com SEO TOOLKIT BEGIN
				$phpbb_seo->prepare_url('pic', $commentrow['image_name'], $image_id, $phpbb_seo->seo_url['album'][$album_id]);
				// www.phpBB-SEO.com SEO TOOLKIT END
	...
	// www.phpBB-SEO.com SEO TOOLKIT BEGIN
	if (!$l_search_title) {
		$l_search_title = ($username ? $username : '' ) . ($u_hilit ? ($username ? ' - ' : '') . $keywords : '');
	}
	// www.phpBB-SEO.com SEO TOOLKIT END

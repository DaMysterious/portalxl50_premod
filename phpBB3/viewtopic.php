...
//-- end: Prime Post Revisions ----------------------------------------------//
	...
	$postrow = array(
		...
		// -- Disable first post signature
		// Mod browser, os & screen
		'USER_BROWSER'		=> ($config['browser_os_enable']) ? switch_agent($row['post_ua']) : '',
		'USER_OS'			=> ($config['browser_os_enable']) ? switch_os($row['post_ua']) : '',
		'USER_SCREEN'		=> ($config['browser_os_enable']) ? switch_screen($row['screen']) : '',
		'S_BROWSERS_TOP'	=> ($config['browser_os_enable'] && $config['browser_os_position'] == 1) ? true : false,
		'S_BROWSERS_PROFILE'=> ($config['browser_os_enable'] && $config['browser_os_position'] == 2) ? true : false,
		// End Mod browser, os & screen		
	    ...
	    // Post links 1.0.1
		'U_MINI_POST_LINK'		=> append_sid(generate_board_url() . "/viewtopic.$phpEx", 'p=' . $row['post_id']) . (($topic_data['topic_type'] == POST_GLOBAL) ? '&amp;f=' . $forum_id : '') . '#p' . $row['post_id'],
		...
		// Start Poster IP
		'POSTER_IP' 		=> (($auth->acl_get('a_') || $auth->acl_get('m_'))) ? $row['poster_ip'] : '',
		// End Poster IP
...
page_header($config['sitename'] . ' : ' . $topic_data['topic_title'] . ' : ' .  $topic_data['forum_name'] . $extra_title, true, $forum_id);
...
{
	if ($config['shout_enable'] && $config['shout_topic'])
	{
		shout_display();
	}
}

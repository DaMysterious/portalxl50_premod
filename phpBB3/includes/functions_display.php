	...
	foreach ($forum_rows as $row)
	{
		...
		if ($row['parent_id'] == $root_data['forum_id'] && $row['forum_type'] == FORUM_CAT)
		{
			$template->assign_block_vars('forumrow', array(
				'S_IS_CAT'				=> true,
				//MOD: Collapse categories
				'S_IS_CAT_HIDDEN'		=> in_array($row['forum_id'], explode("-", $user->data['user_category_collapse'])),
		...
		$sql = 'SELECT f.*, u.*
			...
			WHERE f.forum_last_poster_id = u.user_id
				AND ' . $db->sql_in_set('u.user_id', $row['forum_last_poster_id']);
		$result = $db->sql_query($sql);
		$flag_data = $db->sql_fetchrow($result);
		
		if ($user->data['user_id'] != ANONYMOUS)
		{
			$flag_title = $flag_img = $flag_img_src = '';
			get_user_flag($flag_data['user_country_flag'], $flag_data['user_country'], $flag_title, $flag_img, $flag_img_src);
		}
		$db->sql_freeresult($result);
		// Country Flags Version 3.0.6
		...
		$template->assign_block_vars('forumrow', array(
			'S_IS_CAT'			=> false,
			//MOD: Collapse categories			
			'S_IS_PARENT_CAT_HIDDEN'		=> in_array($row['parent_id'], explode("-", $user->data['user_category_collapse'])),	
			//MOD: Collapse categories			
			...
			'LAST_POSTER_FULL'		=> $last_poster_full . ' ' . $flag_img,
			...
			// Visit counter by forum 1.0.0
			'L_FORUM_VISIT'			=> $user->lang(($online_users['total_online'] > 1) ? 'FORUM_VISITS' : 'FORUM_VISIT', $online_users['total_online']),
			// Visit counter by forum 1.0.0
...
function get_user_avatar($avatar, $avatar_type, $avatar_width, $avatar_height, $alt = 'USER_AVATAR', $ignore_config = false)
{
	...
	{
		$random_avatar = $phpbb_root_path . 'images/avatars/random/no_avatar' . mt_rand(1, 10) . '.gif';
		...
		if (file_exists($random_avatar))
		{
			...
			return '<img src="' . $random_avatar . '" width="' . $avatar_width . '" height="' . $avatar_height . '" title="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" alt="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" />';
		}
		else
		{
		    return '<img src="images/avatars/no_avatar.png" width="' . $avatar_width . '" height="' . $avatar_height . '" title="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" alt="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" />';
			// return '';
		}
	}

...
class auth_admin extends auth
{
	...
	*/
	function display_mask($mode, $permission_type, &$hold_ary, $user_mode = 'user', $local = false, $group_display = true, $cat_data = false)
	{
		...
		if ($local)
		{
			$forum_names_ary =  ($cat_data) ? $cat_data : make_forum_select(false, false, true, false, false, false, true);
		...
		$cur_roles = ($cat_data) ? $kb_auth->acl_role_data($user_mode, $permission_type, array_keys($hold_ary)) : $this->acl_role_data($user_mode, $permission_type, array_keys($hold_ary));

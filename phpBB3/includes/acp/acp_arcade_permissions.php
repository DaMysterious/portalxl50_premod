...
class acp_arcade_permissions
{
	...
	{
		global $db, $user, $auth, $arcade, $auth_arcade, $template, $cache, $arcade_cache;
		...
		if ($mode == 'setting_category_copy')
		{
			$this->tpl_name = 'permission_arcade_category_copy';

			if ($auth->acl_get('a_cauth') && $auth->acl_get('a_authusers') && $auth->acl_get('a_authgroups'))
			{
				$this->page_title = 'ACP_ARCADE_CATEGORY_PERMISSIONS_COPY';
				$this->copy_category_permissions();
				return;
			}
		...
		switch ($mode)
		{
			...
			case 'setting_group_global':
				...
				$this->page_title = ($mode == 'setting_user_global') ? 'ACP_ARCADE_USERS_PERMISSIONS' : 'ACP_ARCADE_GROUPS_PERMISSIONS';
			break;
			...
			case 'view_user_global':
				...
				$this->page_title = 'ACP_VIEW_USER_PERMISSIONS';
			break;
	...
	*/
	function permission_trace($user_id, $cat_id, $permission)
	{
		...
		{
			$sql = 'SELECT u.user_id, u.username, a.user_arcade_permissions, u.user_type
				FROM ' . USERS_TABLE . ' u, ' . ARCADE_USERS_TABLE . ' a
	...
	function copy_category_permissions()
	{
		global $arcade, $auth_arcade, $cache, $template, $user;
		...
		if ($submit)
		{
			$src = request_var('src_cat_id', 0);
			$dest = request_var('dest_cat_ids', array(0));

			if (confirm_box(true))
			{
				if ($arcade->copy_category_permissions($src, $dest))
				{
				...
				{
					trigger_error($user->lang['ARCADE_SELECTED_CAT_NOT_EXIST'] . adm_back_link($this->u_action), E_USER_WARNING);
				}
			}
			else
			{

...
// Colorized Unread Links
						'colorized_links'		=> array('lang' => 'COLORIZED_LINKS',        'validate' => 'string', 'type' => 'text:7:7', 'explain' => true),
// Colorized Unread Links
			...
			case 'post':
				$display_vars = array(
					...
					'vars'	=> array(
						...
						'browser_os_enable'		=> array('lang' => 'ALLOW_BROWSER_OS',		'validate' => 'bool',	'type' => 'radio:yes_no', 'explain' => true),
						'browser_os_position'	=> array('lang' => 'ALLOW_BROWSER_POS',		'validate' => 'int',	'type' => 'select', 'method' => 'browser_os_select', 'explain' => true),
...
//-- end: Prime Trash Bin (Topics) ------------------------------------------//
	...
	*/
	...
	/**
	* Select browser & os action
	*/
	function browser_os_select($value, $key = '')
	{
		global $user;

		return '<option value="1" title="' .$user->lang['BROWSER_OS_TOP']. '"' . (($value == 1) ? ' selected="selected"' : '') . '>' .$user->lang['BROWSER_OS_TOP']. '</option><option value="2" title="' .$user->lang['BROWSER_OS_PROFILE']. '"' . (($value == 2) ? ' selected="selected"' : '') . '>' .$user->lang['BROWSER_OS_PROFILE']. '</option>';
	}

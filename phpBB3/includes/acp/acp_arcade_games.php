...
class acp_arcade_games
{
	...
	{
		global $db, $user, $auth, $auth_arcade, $template, $cache, $arcade, $arcade_config, $arcade_cache;
	...
	function add_games()
	{
		...
		global $config, $arcade_config, $phpbb_admin_path, $phpbb_root_path, $phpEx, $mode, $file_functions;
		...
		if ($update)
		{
			...
			foreach ($game_array as $game)
			{
				...
				add_log('admin', 'LOG_ARCADE_ADD_GAME', '<a onclick="window.open(this.href); return false;" href="'. $arcade->url('mode=play&amp;g=' . $game_data['game_id']) .'">'. $game_data['game_name'] .'</a>');

				if ($arcade_config['announce_game'])
				{
					$arcade->create_announcement($game_data, $arcade_config['announce_forum']);
				}
		...
		$dir = $phpbb_root_path . $arcade_config['game_path'];
		...
		$games = $games_add = $games_name = array();
		...
		if ($files = scandir($dir))
		{
			foreach ($files as $file)
			{
				...
				$game_file 		= $dir . $file . '/'. $file .'.swf';
				$install_file 	= $dir . $file . '/'. $file .'.' . $phpEx;
				$install_xml 	= $dir . $file . '/game.xml';
				// Check to make sure a flash file exists before including the game install file
				if (file_exists($game_file))
				{
					...
					if (file_exists($install_file))
					{
						...
						if (isset($game_data))
						{
							...
							if (isset($games_name[$game_data['game_name']]) || isset($games_add[$game_data['game_name']]))
							{
								$duplicate_games = 0;
								$temp_name = $game_data['game_name'];
								while (isset($games_name[$game_data['game_name']]) || isset($games_add[$game_data['game_name']]))
								{
									$duplicate_games++;
	...
	function edit_games()
	{
		...
		switch ($action)
		{
			case 'edit':
				...
				if ($update)
				{
					add_log('admin', 'LOG_ARCADE_EDIT_GAMES', '<a onclick="window.open(this.href); return false;" href="'. $arcade->url('mode=play&amp;g=' . $this->new_game_data['game_id']) .'">'. $this->new_game_data['game_name'] .'</a>');
					$cache->destroy('sql', ARCADE_CATS_TABLE);
					$cache->destroy('sql', ARCADE_GAMES_TABLE);
					$cache->destroy('sql', ARCADE_SCORES_TABLE);
					...
					$manage_games_link	= $auth->acl_get('a_arcade_cat') ? '<br /><br /><a href="' . append_sid("{$phpbb_admin_path}index.$phpEx", "i=arcade_manage&amp;mode=manage&amp;parent_id={$old_cat_id}" . (($start) ? "&amp;start=$start" : '')) . '">&laquo; ' . $user->lang['ACP_BACK_MANAGE_GAMES'] . '</a>' : '';
					$game_return_links = '<br /><br />' . sprintf($user->lang['ARCADE_RETURN_LINKS'], '<a href="' . $arcade->url("mode=play&amp;g={$game_id}") . '">&laquo; ', $this->new_game_data['game_name'], '</a>', '<a href="' . $arcade->url('mode=cat&amp;c=' . $this->new_game_data['cat_id']) . '&amp;g=' . $game_id .'#g' . $game_id .'">&laquo; ', $this->new_game_data['cat_name'], '</a>', '<a href="' . $arcade->url() . '">&laquo; ', '</a>');

					if ($update_install_file)
					{
						trigger_error($user->lang['ARCADE_GAME_UPDATED'] . $edit_games_link . $manage_games_link . $game_return_links);
					}
					else
					{
						trigger_error($user->lang['ARCADE_GAME_UPDATED_ERROR'] . $edit_games_link . $manage_games_link . $game_return_links, E_USER_WARNING);
					}
				}
			...
			case 'delete':
				if (confirm_box(true))
				{
					...
					{
						// language frontier
						if (request_var('delete_files', '') === $user->lang['YES'])
						{
							$delete_files = true;
						}
					}
					...
					$error = $arcade->delete_game($game_id, $game_data['cat_id'], $delete_files);
				...
				else
				{
					...
					confirm_box(false, 'DELETE_SELECTED_GAME', build_hidden_fields($s_hidden_fields), 'confirm_body_delete_arcade_games.html');
				}
			break;
		}
	...
	function unpack_games()
	{
		...
		if ($update)
		{
			...
			$check_dirs = array('store/', 'games/', 'arcade/gamedata/', $arcade_config['game_path'], $arcade_config['unpack_game_path']);
			...
			foreach ($check_dirs as $dir)
			{
				...
				if (!file_exists($path))
				{
					$errors[] = sprintf($user->lang['NO_DIRECTORY'], $dir);
				}
			...
			// Add game to database
			$bad_game = $bad_perm_game = array();
			foreach ($game_array as $key => $game)
			{
				$use_method = strrchr($game, '.');
				if ($use_method == '.zip')
				{
					$compress = new compress_zip('r', $phpbb_root_path . $arcade_config['unpack_game_path'] . $game);
				}
				else
				{
					$compress = new compress_tar('r', $phpbb_root_path . $arcade_config['unpack_game_path'] . $game);
				}
				...
				if (is_dir($phpbb_root_path . $tmp_path . 'games'))
				{
					if ((is_dir($phpbb_root_path . 'games') && !phpbb_is_writable($phpbb_root_path . 'games')) || (!is_dir($phpbb_root_path . 'games') && !phpbb_is_writable($phpbb_root_path)))
					{
						$bad_perm_game[] = $game;
				...
				unlink($phpbb_root_path . $arcade_config['unpack_game_path'] . $game);
				...
				if (file_exists($phpbb_root_path . $game_data_path . 'games'))
				{
					//Copy and delete
					$file_functions->copy_dir($phpbb_root_path . $game_data_path . 'games', $phpbb_root_path . 'games');
					$file_functions->delete_dir($phpbb_root_path . $game_data_path . 'games');
				}

				// Convert the ibp or xml install file
				$game_type = $arcade->convert_install_file($game_config, $is_xml);

				$file_functions->copy_dir($phpbb_root_path . $game_file_path, $phpbb_root_path . $arcade_config['game_path'] . $game_name);
				$file_functions->delete_dir($phpbb_root_path . $tmp_path);
			...
			$s_bad_perm_game = sizeof($bad_perm_game);
			...
			if ($s_bad_perm_game)
			{
				$lang = ($s_bad_perm_game > 1) ? $user->lang['ARCADE_UNPACK_GAMES_PERM_ERROR'] : $user->lang['ARCADE_UNPACK_GAME_PERM_ERROR'];
				$message .= sprintf($lang, implode(', ', $bad_perm_game));
			}
		...
		$dir = $phpbb_root_path . $arcade_config['unpack_game_path'];
			...
			foreach ($files as $file)
			{
				...
				if ($found)
				{
					...
					$sql = 'SELECT game_name
						FROM '. ARCADE_GAMES_TABLE . "
						WHERE game_swf = '" . $db->sql_escape($filename . '.swf') . "'";
		...
		$can_upload = (file_exists($phpbb_root_path . $arcade_config['unpack_game_path']) && @is_writable($phpbb_root_path . $arcade_config['unpack_game_path']) && (@ini_get('file_uploads') || strtolower(@ini_get('file_uploads')) == 'on')) ? true : false;
		$template->assign_vars(array(
	...
	function edit_scores()
	{
		...
		if ($update && $action == 'edit')
		{
			...
			add_log('admin', 'LOG_ARCADE_EDIT_SCORE', $username, '<a onclick="window.open(this.href); return false;" href="'. $arcade->url('mode=play&amp;g=' . $game_data['game_id']) .'">'. $game_data['game_name'] .'</a>');
			trigger_error(sprintf($user->lang['ARCADE_SCORE_UPDATED'], $username, $game_data['game_name']) . adm_back_link($this->u_action));
		}

		$action = (isset($_POST['reset_user_scores'])) ? 'reset_user_scores' : $action;
		$action = (isset($_POST['reset_user_all']))    ? 'reset_user_all' : $action;
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				$sql_array = array(
					...
					'WHERE'		=> 's.user_id = ' . (int) $user_id . ' AND g.game_id = ' . (int) $game_id,
				);
			...
			case 'show_scores':
				...
				$template->assign_vars(array(
					'S_ON_PAGE'	=> on_page($total_scores, $arcade_config['acp_items_per_page'], $start),
					'PAGINATION'	=> generate_pagination($this->u_action . '&amp;action=show_scores&amp;g=' . $game_id, $total_scores, $arcade_config['acp_items_per_page'], $start, true),
				));
		...
		while($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('games', array(
				...
				'GAME_IMAGE'	=> ($row['game_image']) ? '<img src="' . $arcade->url("img={$row['game_image']}") . '" width="25" height="25" alt="" />' : '',
		...
		$template->assign_vars(array(
			...
			'S_SHOW_POINTS'		=> ($arcade->points['installed'] && $auth->acl_get('a_arcade_points_settings')),
		...
		$template->assign_vars(array(
			'S_ON_PAGE'		=> on_page($total_scores, $arcade_config['acp_items_per_page'], $start),
			'PAGINATION'	=> generate_pagination($this->u_action, $total_scores, $arcade_config['acp_items_per_page'], $start, true),
		));
	}
	...
	*/
	...
	{
		global $phpbb_root_path, $config, $arcade_config, $db, $user, $phpEx, $arcade;
		...
		$file->move_file($arcade_config['unpack_game_path'], true, true);

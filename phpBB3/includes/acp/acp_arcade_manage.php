...
class acp_arcade_manage
{
	...
	{
		global $db, $user, $auth, $auth_arcade, $template, $cache, $arcade, $arcade_cache;
		global $config, $arcade_config, $phpbb_admin_path, $phpbb_root_path, $phpEx, $file_functions;
		...
		$confirm_action 	= array('delete_marked', 'g_delete', 'reset_marked', 'reset_scores_marked', 'move_marked');
		...
		switch ($action)
		{
			...
			case 'reset_scores_marked':
				if (!$auth->acl_get('a_arcade_scores'))
				{
					trigger_error($user->lang['NO_PERMISSION_GAME_' . (($action == 'reset_marked') ? 'RESET' : 'SCORE_RESET')] . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id), E_USER_WARNING);
				}
			break;
		...
		if ($update)
		{
			switch ($action)
			{
				case 'delete':
					...
					$sub_delete_files = false;
					if (isset($_POST['sub_delete_files']))
					{
						// language frontier
						if (request_var('sub_delete_files', '') === $user->lang['YES'])
						{
							$sub_delete_files = true;
						}
					}

					$errors = $this->delete_cat($cat_id, $action_games, $delete_files, $action_subcats, $sub_delete_files, $games_to_id, $subcats_to_id);
				...
				case 'g_delete':
					...
					if (confirm_box(true))
					{
						...
						$errors = $arcade->delete_game($game_id, $game_data['cat_id'], $delete_files);
				...
				case 'reset_scores_marked':
					if (confirm_box(true))
					{
						...
						$message = sprintf($user->lang['GAME' . ((sizeof($game_data_array) > 1) ? 'S' : '') . '_' . (($action == 'reset_marked') ? 'RESET' : 'SCORES_RESET')], $game_name_list);

						$errors = $arcade->reset_game($action, $game_ids, $this->parent_id);
						if (sizeof($errors))
						{
							break;
						}

						$reset_log = (sizeof($game_data_array) > 1) ? 'LOG_ARCADE_RESET_GAMES' : 'LOG_ARCADE_RESET_GAME';

						add_log('admin', $reset_log . (($action == 'reset_marked') ? '' : '_SCORES'), $game_name_list);
						$cache->destroy('sql', ARCADE_CATS_TABLE);
						$cache->destroy('sql', ARCADE_GAMES_TABLE);
						$cache->destroy('sql', ARCADE_SCORES_TABLE);
				...
				case 'delete_marked':
					...
					if (confirm_box(true))
					{
						...
						$game_data = $arcade->get_game_data($game_ids);
						$errors = $arcade->delete_game($game_ids, $this->parent_id, $delete_files);
				...
				case 'add':
					...
					if (!sizeof($errors))
					{
						...
						{
							$arcade->copy_category_permissions($cat_perm_from, $cat_data['cat_id'], ($action == 'edit') ? true : false);
						}
		...
		switch ($action)
		{
			...
			case 'delete_marked':
				...
				confirm_box(false, 'DELETE_SELECTED_GAMES', build_hidden_fields($s_hidden_fields), 'confirm_body_delete_arcade_games.html');
			...
			case 'reset_scores_marked':
				...
				$confirm = (sizeof($game_ids) == 1) ? 'RESET_SELECTED_GAME' : 'RESET_SELECTED_GAMES';

				confirm_box(false, $confirm . (($action == 'reset_marked') ? '' : '_SCORES'), build_hidden_fields($s_hidden_fields));
			break;
			...
			case 'edit':
				...
				{
					$template->assign_vars(array(
						'S_MOVE_CAT_OPTIONS'		=> $arcade->make_cat_select($cat_data['parent_id'], $cat_id, false, true, false))
					);
				}
				...
				$template->assign_vars(array(
					...
					'S_IMAGE_BASEDIR'			=> $phpbb_root_path . $arcade_config['cat_image_path'],
					'CAT_IMAGE_SRC'				=> ($cat_data['cat_image']) ? $phpbb_root_path . $arcade_config['cat_image_path'] . $cat_data['cat_image'] : $phpbb_root_path . 'images/spacer.gif',
		...
		if ($row = $db->sql_fetchrow($result))
		{
			do
			{
				...
				$template->assign_block_vars('cats', array(
					...
					'CAT_IMAGE'			=> ($row['cat_image']) ? '<img src="' . $phpbb_root_path . $arcade_config['cat_image_path'] . $row['cat_image'] . '" alt="" />' : '',
		...
		{
			$start = $start - $arcade_config['acp_items_per_page'];
		...
		if ($total_games && $this->parent_id)
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				do
				{
					...
					$template->assign_block_vars('games', array(
						...
						'GAME_IMAGE'		=> ($row['game_image']) ? '<img src="' . $arcade->url("img={$row['game_image']}") . '" width="50" height="50" alt="" />' : '',

						'U_GAME_PLAY'     	=> $arcade->url("mode=play&amp;g={$row['game_id']}"),
						'U_MOVE_UP'			=> $url . '&amp;action=g_move_up',
						'U_MOVE_DOWN'		=> $url . '&amp;action=g_move_down',
						'U_EDIT'			=> append_sid("{$phpbb_admin_path}index.$phpEx", "i=arcade_games&amp;mode=edit_games&amp;action=edit&amp;g={$row['game_id']}" . (($start) ? "&amp;start=$start" : '')),
			...
			$mark_options = array('sync_marked', 'reset_scores_marked', 'reset_marked', 'move_marked', 'delete_marked');

			if ((!$auth->acl_get('a_arcade_delete_game') && $key = array_search('delete_marked', $mark_options)) || (!$auth->acl_get('a_arcade_scores') && ($key = array_search('reset_marked', $mark_options) || $key = array_search('reset_scores_marked', $mark_options))))
			{
			...
			$template->assign_vars(array(
				...
				'S_ON_PAGE'			=> on_page($total_games, $arcade_config['acp_items_per_page'], $start),
				'PAGINATION'		=> generate_pagination($this->u_action . '&amp;parent_id=' . $this->parent_id, $total_games, $arcade_config['acp_items_per_page'], $start, true))
			);
			...
			// Quick jump of all the games in this category.
			foreach ($row as $game)
			{
		...
		else
		{
			...
			if ($row['cat_type'] == ARCADE_CAT_GAMES && $row['cat_type'] != $cat_data_sql['cat_type'])
			{
				...
				else if ($cat_data_sql['type_action'] == 'delete')
				{
					...
					$errors = $this->delete_cat_content($cat_data_sql['cat_id'], $delete_files);
				}
				else
				{
				...
				if ($row['right_id'] - $row['left_id'] > 1)
				{
					...
					if ($action_subcats == 'delete')
					{
						...
						foreach ($rows as $_row)
						{
							...
							$cat_ids[] = $_row['cat_id'];
							$errors = array_merge($errors, $this->delete_cat_content($_row['cat_id'], $sub_delete_files));
						}
	...
	*/
	function delete_cat($cat_id, $action_games = 'delete', $delete_files = false, $action_subcats = 'delete', $sub_delete_files = false, $games_to_id = 0, $subcats_to_id = 0)
	{
		global $db, $user, $cache, $arcade;
		...
		if ($action_games == 'delete')
		{
			...
			$errors = array_merge($errors, $this->delete_cat_content($cat_id, $delete_files));
		}
		...
		if ($action_subcats == 'delete')
		{
			...
			{
				$cat_ids[] = $row['cat_id'];
				$errors = array_merge($errors, $this->delete_cat_content($row['cat_id'], $sub_delete_files));
			}
	...
	*/
	function delete_cat_content($cat_id, $delete_files = false)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$delete_games[$cat_id][] = $row['game_id'];
		}
		$db->sql_freeresult($result);

		if (sizeof($delete_games))
		{
			foreach ($delete_games as $cat_id => $game_ids)
			{
				$errors = $arcade->delete_game($game_ids, $cat_id, $delete_files, false);
			}
		}

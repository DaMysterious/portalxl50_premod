...
class ucp_arcade
{
	...
	{
		global $db, $user, $auth_arcade, $arcade, $template, $phpbb_root_path, $phpEx, $arcade_config;
	...
	{
		global $db, $user, $arcade, $arcade_config, $template, $phpbb_root_path, $phpEx;
		...
		if ($arcade_config['send_arcade_pm'])
		{
		...
		if ($submit)
		{
			$validate_array = array(
				...
				'games_per_page'	=> array('num', false, ARCADE_UCP_GAMES_PER_PAGE_MIN, ARCADE_UCP_GAMES_PER_PAGE_MAX),
			);
		...
		if ($arcade_config['override_user_sort'])
		{
	...
	function manage_favorites()
	{
		...
		$games_per_page = $arcade_config['games_per_page'];
		...
		foreach ($row as $item)
		{
			$template->assign_block_vars('games', array(
				'U_GAME'		=> $arcade->url("mode=play&amp;g={$item['game_id']}"),
				'U_CAT'			=> $arcade->url('mode=cat&amp;c='  . $item['cat_id']),

...
//-- end : log connections --------------------------------------------------------
		...
		if (isset($config['shout_version']))
		{
			...
			{
				if ($this->data['user_id'] != ANONYMOUS && !$bot && $this->data['user_lastvisit'] == 0)
				{
					shout_add_newest_user((int)$this->data['user_id']);
				}
				if ($this->data['user_id'] != ANONYMOUS && $viewonline && !$set_admin && $config['shout_sessions'])
				{
					post_session_shout((int)$this->data['user_id'], (string)$this->ip, (int)$bot);
				}
			}
		}
		// End Breizh Shoutbox

...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true, $update_search_index = true)
{
	...
	if (isset($data['dl_topic_user']) && $data['dl_topic_user'])
	{
	...
	switch ($post_mode)
	{
		...
		case 'reply':
			$sql_data[POSTS_TABLE]['sql'] = array(
				...
				'screen'			=> $data['screen'],
				// 'screen'         	=> (!empty($data['screen'])) ? 1024 : 1280, // temporary fix
...
//-- end: Prime Notify ------------------------------------------------------//		
	...
	{
		$is_global = ($topic_type == POST_GLOBAL) ? true : false;
		if ($config['shout_exclude_forums'])
		{
			$exclude_forums = array();
			$exclude_forums = explode(',', $config['shout_exclude_forums']);
			if (!in_array($data['forum_id'], $exclude_forums))
			{
				advert_post_shoutbox($user->data['user_id'], $user->ip, $data['topic_id'], $data['topic_title'], $data['forum_id'], $url, $post_mode, $topic_type, $post_approval);
			}
		}
		else
		{

...
class auth
{
	...
	*/
	function acl_get_list($user_id = false, $opts = false, $forum_id = false, $schedule_override = false)
	{
		...
		{
			$hold_ary = $this->acl_raw_data($user_id, $opts, $forum_id, $schedule_override);
		}
	...
	*/
	function acl_clear_prefetch($user_id = false)
	{
		...
		$sql = 'UPDATE ' . USERS_TABLE . "
			SET user_permissions = '', user_kb_permissions = '',
				user_perm_from = 0
			$where_sql";
		$db->sql_query($sql);
	...
	*/
	function acl_raw_data($user_id = false, $opts = false, $forum_id = false, $schedule_override = false)
	{
		global $db;
		...
		foreach ($sql_ary as $sql)
		{
			...
			{
				//BEGIN - Scheduled Group Membership
				//Not time yet so skip this auth
				if (!group_membership_scheduled($row) && !$schedule_override)
				{
					continue;
				}
				//END - Scheduled Group Membership
	...
	*/
	function acl_raw_data_single_user($user_id)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			//BEGIN - Scheduled Group Membership
			//Not time yet so skip this auth
			if (!group_membership_scheduled($row))
			{
				continue;
			}
			//END - Scheduled Group Membership

		...
		for ($j = $i; $j < ($i + $cats_per_row); $j++)
		{
			...
			if(isset($subcats[$cat_id]) && $config['kb_list_subcats'])
			{
				foreach($subcats[$cat_id] as $subcat_id => $subcat)
				{
					...
					$s_subcats_list[] = '<div><a href="' . kb_append_sid('cat', array('id' => $subcat_id, 'title' => $subcat['name'])) . '">' . $subcat['name'] . '</a>';
				}
				$s_subcats_list = implode(', </div>', $s_subcats_list);
			}

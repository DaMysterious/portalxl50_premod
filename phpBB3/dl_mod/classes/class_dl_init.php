...
* @file				class_dl_init.php 3 2012/12/29 OXPUS
...
class dl_init extends dl_mod
{
	...
	public static function init()
	{
		...
		*/
		if ($config['dl_use_ext_blacklist'])
		{
			...
			{
				$ext_blacklist = array_unique(dl_cache::obtain_dl_blacklist());
			}
		}
		...
		*/
		...
		$user_timezone	= $user->data['user_timezone'] * 3600;
		$user_dst		= $user->data['user_dst'] * 3600;
		$zone_offset	= $user_timezone + $user_dst;
		$current_month	= @gmdate('Ym', time() + $zone_offset);
		...
		*/
		if (isset($config['dl_traffic_retime']) && !$config['dl_traffic_off'])
		{
			...
			if ($auto_overall_traffic_month < $current_month)
			{
				...
				set_config('dl_traffic_retime', $config['dl_traffic_retime'], false);
			}
		}
		...
		*/
		if (isset($config['dl_click_reset_time']))
		{
			...
			if ($auto_click_reset_month < $current_month)
			{
				...
				set_config('dl_click_reset_time', time(), false);
			}
		}
		...
		*/
		...
		$sql = 'SELECT ban_id FROM ' . DL_BANLIST_TABLE . '
			WHERE user_id = ' . (int) $user_id . "
				...
				OR user_agent " . $db->sql_like_expression(self::dl_client($user_client)) . "
	...
	public static function dl_client($client)
	{
		$browser_name = 'n/a';

		if (file_exists(self::phpbb_root_path() . 'dl_mod/includes/dl_user_agents' . self::phpEx()) && $client)
		{
			include(self::phpbb_root_path() . 'dl_mod/includes/dl_user_agents' . self::phpEx());
		}
		else
		{
			return $browser_name;
